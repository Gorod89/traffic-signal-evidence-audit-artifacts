...
The `agent` class is the one that must not be trusted without a check, because
nothing prevents a summariser from transcribing a number incorrectly. This
...
eplus = load_json(Path(__file__).resolve().parent / "eplus" / "runs" / "EPLUS_FINAL.json")
if eplus:
...
for name in ("dplus_summary.json", "headline_comparison.csv"):
    p = Path(__file__).resolve().parent / "dplus" / name
    if p.exists():
        dplus = p
        break
dplus = Path(__file__).resolve().parent / "dplus" / "headline_comparison.csv"
if dplus.exists():
    ...
    trials_csv = Path(__file__).resolve().parent / "dplus" / "trials_candidate_level.csv"
    ...
    mw = Path(__file__).resolve().parent / "dplus" / "matched_worst_condition.csv"
    if mw.exists():
