...
This repository is a curated public companion to the evidence audit. It is not a complete copy of the working archive and does not claim full numerical reproducibility of every reported table.

## Included

1. Corrected evidence tools and their tests.
2. Portable Phase-0 analysis and integrity scripts.
3. Selected Phase-0 derived JSON records and the pinned V6 paired derivative.
4. The prediction-lock record, its digest, and the reviewed population contract.
5. Retained Phase 1 and Phase 2 scripts and portable result JSONs.
...
- article source, article PDF, bibliography, journal template, class/style files, and journal logos;
- V1, V3, and V4 raw trace archives;
...
`docs/ARTIFACT_MAP.csv` uses four explicit states:

- `runnable_public`: the public script, required input, and output are present;
- `inspection_only_public`: script and derived output are present, but at least one raw input or environment is absent;
- `private_archive_only`: evidence is retained privately but not released;
- `not_covered`: the public audit does not test this manuscript claim.

No absent branch is silently removed from an audit denominator.
